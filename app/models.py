...
class User(UserMixin, db.Model):
    ...
    role = db.Column(db.String(20), nullable=False)  # admin, secretary, applicant, contact_person
    ...
    def is_contact_person(self):
        """Check if user has contact_person role (入党联系人)."""
        return self.role == 'contact_person'
...
class Application(db.Model):
    ...
    contact_person_id = db.Column(db.Integer, db.ForeignKey('users.id'))  # 入党联系人
    ...
    contact_person = db.relationship('User', foreign_keys=[contact_person_id])
...
class Document(db.Model):
    ...
    # Relationship for reviewer (need foreign_keys to disambiguate from uploader)
